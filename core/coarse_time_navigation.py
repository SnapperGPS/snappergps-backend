...
def positioning_simplified(snapshot_idx_dict,
    ...
    Inputs:
        ...
        n_failed - Number of position fixes that failed before 1st snapshot,
                   default=0
        last_plausible_utc - Timestamp of last plausible fix in UTC,
                             default=None
        max_vel - Velocity to calculate maximum spatial distance between 2
                  consecutive fixes to be plausible [m/s], default=np.inf
        max_time_drift - Time drift to calculate maximum temporal distance
                         between 2 consecutive fixes to be plausible
                         (dimensionless), i.e., the change of the coarse-time
                         error, default=np.inf
    ...
    if ls_mode is not None:
        ...
        reliability_vec = {gnss: classifier[gnss].predict_log_proba(
            snr_dict[gnss].reshape(-1, 1))[:, 1]
            if len(snr_dict[gnss]) > 0 else np.array([])
            for gnss in gnss_list}
            ...
            while not plausible_solution and not iterator_empty:
                try:
                    ...
                    try:
                        ...
                        if np.isnan(hdop):
                            hdop = np.inf
                        # Check plausibility of solution by checking
                        # pseudorange residuals and distance to last plausible
                        # solution
    ...
    state = {"n_failed": n_failed,
             ...
             "last_plausible_utc": last_plausible_utc}
